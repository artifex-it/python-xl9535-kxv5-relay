...
"""
Python XL9535 KxV5 Relay
https://github.com/artifex-it/python-xl9535-kxv5-relay
...
# registers
# XL9535_INPUT_PORT0 = 0x00
# XL9535_INPUT_PORT1 = 0x01
XL9535_OUTPUT_PORT0 = 0x02
# XL9535_OUTPUT_PORT1 = 0x03
XL9535_INVERSION_PORT0 = 0x04
# XL9535_INVERSION_PORT1 = 0x05
XL9535_CONFIG_PORT0 = 0x06
# XL9535_CONFIG_PORT1 = 0x07
...
class XL9535_KXV5:
	...
	def init(self):
		...
		self._i2c.writeto(self._address, bytes([XL9535_INVERSION_PORT0]) + self._buf2)

		# all relays (outputs) off (NC<->COM-x-NO), blue LED off
		# high bit means relay on (NC-x-COM<->NO), blue LED on
		self._i2c.writeto(self._address, bytes([XL9535_OUTPUT_PORT0]) + self._buf2)

		# all circuits (configs) enabled
		# low bit means enabled
		self._i2c.writeto(self._address, bytes([XL9535_CONFIG_PORT0]) + self._buf2)
	...
	def relay(self, num, value=None):
		...
		p = num // 8
		b = 1 << (num % 8)
		self._i2c.writeto_then_readfrom(self._address, bytes([XL9535_OUTPUT_PORT0]), self._buf2)
		if value is None:
			# get relay status
	...
	def circuit(self, num, value=None):
		...
		p = num // 8
		b = 1 << (num % 8)
		self._i2c.writeto_then_readfrom(self._address, bytes([XL9535_CONFIG_PORT0]), self._buf2)
		self._buf2[0] ^= 0xFF
		self._buf2[1] ^= 0xFF
		if value is None:
			# get circuit status
